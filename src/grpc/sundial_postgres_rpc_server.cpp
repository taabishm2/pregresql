...
class SundialPostgresRPCServerImpl final : public SundialPostgresRPC::Service {
  Status getClassCatalog(ServerContext* context, const ClassCatalogRequest* request,
                  ClassCatalogResponse* reply) override {
    std::cout << "received message: "<< request->relname() << std::endl;
    std::string prefix("PG_CLASS ");
    reply->set_relname(prefix + request->relname());
    return Status::OK;
  }
};

void RunServer() {
  std::string server_address = absl::StrFormat("0.0.0.0:50052");
  SundialPostgresRPCServerImpl service;
